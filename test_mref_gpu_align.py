...
cu_module.mref_align_run.restype = ctypes.c_ulonglong
# cu_module.get_gpu_img_ptr.restype = ctypes.c_ulonglong
# cu_module.get_gpu_img_ptr.restype = ctypes.POINTER(ctypes.c_float)
# cu_module.get_gpu_img_ptr.restype = ctypes.POINTER(ctypes.c_void_p)
...
def mref_ali2d_gpu(
    ...
    while Iter < max_iter and again:
        ...
        #
        # FOR gpu_batch_i DO ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ >        
        gpu_ref_sum = cp.zeros((numref, 2, nx, nx), cp.float32)
        for gpu_batch_idx in range(gpu_batch_count):
            ...
            # Get ptr of alligned images
            RangePush("Batch alignment")
            gpu_img_ptr = cu_module.mref_align_run( ctypes.c_int(gpu_batch_start), ctypes.c_int(gpu_batch_end))
            RangePop()
            gpu_refs = cp.asarray([gpu_aln_param[k].ref_id for k in range(gpu_batch_start,gpu_batch_end)])
            RangePush("Batch sum oe fast cp.sum")
            gpu_ref_sum += kernel_sum_oe(gpu_img_ptr, gpu_batch_end-gpu_batch_start, nx, gpu_refs, numref)
            RangePop()
            # print progress bar
        ...
        for iref in range(numref):
            for it in [0,1]:
                Util.add_img(refi[iref][it], EMNumPy.numpy2em(ref_sum[iref][it]))
            
        RangePop()
        # ------------------------------------------------------------
#         print("="*30)
#         print("gpu: 10,5,75:85 ", gpu_batch_img[10,5,75:85])
#         print("cpu: 10,5,75:85 ", data[10].numpy()[5,75:85])
#         print("="*30)

        #===========
        RangePush("MPI average")
        for j in range(numref):
